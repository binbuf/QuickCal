...
### Added
- **DMG install background** — the disk image now shows a drag-to-Applications hint
  and spells out the one-time first-open step
  (`xattr -dr com.apple.quarantine /Applications/QuickCal.app`) directly in the
  window, so the un-notarized "damaged" Gatekeeper warning no longer catches anyone
  off guard. Generated by a committed, reproducible script
  (`Scripts/dmg/make-background.swift`) and wired into the release workflow.
...
[0.2.2]: https://github.com/binbuf/QuickCal/compare/v0.2.1...v0.2.2
